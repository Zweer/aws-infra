import { NestedStack, type NestedStackProps, type Stack } from 'aws-cdk-lib';
import {
  ARecord,
  CnameRecord,
...
export class DnsStack extends NestedStack {
  ...
  protected createPidgeyRecords(): void {
    const target = RecordTarget.fromIpAddresses('192.243.102.171');

    this.createARecords('Pidgey', [
      { recordName: 'pidgey', target },
      { recordName: 'valvometal', target },
    ]);
  }

  protected createMailtrapRecords(): void {
    this.createCnames('Mailtrap', [
      { recordName: 'mt-link', domainName: 't.mailtrap.live.' },
      { recordName: 'mt81', domainName: 'smtp.mailtrap.live.' },
      { recordName: 'rwmt1._domainkey', domainName: 'rwmt1.dkim.smtp.mailtrap.live.' },
      { recordName: 'rwmt2._domainkey', domainName: 'rwmt2.dkim.smtp.mailtrap.live.' },
    ]);
    ...
    this.createTxtRecords('Mailtrap', [
      {
        recordName: '_dmarc',
        value:
          'v=DMARC1; p=none; rua=mailto:dmarc@smtp.mailtrap.live; ruf=mailto:dmarc@smtp.mailtrap.live; rf=afrf; pct=100',
      },
    ]);
  }

  protected createGmailRecords(): void {
  ...
  protected createARecords(
    subject: string,
    records: { recordName: string; target: RecordTarget }[],
  ): void {
    for (const { recordName, target } of records) {
  ...
  protected createCnames(
    subject: string,
    records: { recordName: string; domainName: string }[],
  ): void {
    for (const { recordName, domainName } of records) {
  ...
  protected createTxtRecords(
    subject: string,
    records: { recordName: string; value: string }[],
  ): void {
    for (const { recordName, value } of records) {
